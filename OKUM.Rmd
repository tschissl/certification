...
plot_youd <- function(a, y, z) { # a = measurand, y and z ref
  ...
  p <- ggplot(RM, aes(RM1, RM2, label = GOM.median.Lab))  + xlim(-5, 5) + ylim(-5,5) + 
    ...
    geom_errorbar(aes(ymin=RM2-RM2.s, ymax=RM2+RM2.s)) + 
    geom_errorbarh(aes(xmin=RM1-RM1.s, xmax=RM1+RM1.s))
...
The between-laboratory variance for `r measurand` is `r sL2`   
The between-bottle variance for `r measurand` is `r sbb2`   
The repeatability variance for `r measurand` is `r sr2`    
The standard uncertainty for the assigned value of `r measurand` is `r u2`  

### after outlier rejection
The between-laboratory variance for `r measurand` is `r sL2.a`   
The between-bottle variance for `r measurand` is `r sbb2.a`   
The repeatability variance for `r measurand` is `r sr2.a`    
The standard uncertainty for the property value of `r measurand` is `r u1.a`  
The standard uncertainty for the property value of `r measurand` is `r u2.a`  (check with different way of calculation)
...
The `r measurand` median of the Lab+packet medians after outlier removal is `r round(median.after, 4)` g/100 g   
The expanded standard uncertainty for the assigned value of `r measurand` is `r u2.a*t.value` 
