...
measurand.name <- 'FeO' # defining the measurand
``` 
...
# Data for certification project were gathered and joined in Excel. The files were exported from Excel as xxxx.csv files to make the universially readable. For this markdown the data is stored in the "root/documents" directory.
...
```{r methods and lab plot as is, fig.width=8, fig.height=4}
plot_method(measurand); plot_lab(measurand, MorT, horw=TRUE, u =FALSE) 
```
```{r fig.width=6, fig.height=6}
plot_youd(measurand.name, 'GAS', 'OKUM'); plot_youd(measurand.name, 'MUH', 'OKUM')
```
...
# outlier <- ifelse(OKUM.measurand[[measurand.name]]=="omit", "X", OKUM.measurand[[measurand.name]]) # does not work for FeO
...
The **`r measurand.name`** median of the Lab+packet medians after outlier removal is **`r round(median.after, 4)`** `r unit`   
The expanded standard uncertainty for the assigned value of `r measurand.name` is **`r u2.a*t.value`** with k = `r round(t.value, 2)`   
exluded labs for `r measurand.name` is/are `r outlier`  
labs remaining for calculations `r p`  
#### comments

outliers:|lab#
--------|-----
based on Youden plots:|
based on Mandel's k performance:|
based on PP-XRF:|
